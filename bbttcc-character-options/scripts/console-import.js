// BBTTCC Character Options Console Import Script
// Copy and paste this entire script into the Foundry VTT console, then run: await importBBTTCCOptions()

async function importBBTTCCOptions() {
    console.log('BBTTCC | Starting character options import...');
    
    try {
        // Import Character Archetypes
        await importCharacterArchetypes();
        
        // Import Crew Types  
        await importCrewTypes();
        
        console.log('BBTTCC | All character options imported successfully!');
        ui.notifications.info('BBTTCC Character Options imported successfully!');
        
    } catch (error) {
        console.error('BBTTCC | Import failed:', error);
        ui.notifications.error('Failed to import character options. Check console for details.');
    }
}

async function importCharacterArchetypes() {
    const pack = game.packs.get('bbttcc-character-options.character-archetypes');
    if (!pack) throw new Error('Character archetypes pack not found');
    
    console.log('BBTTCC | Importing Character Archetypes...');
    ...
    const createData = archetypes.map(item => ({ ...item, folder: null, sort: 0 }));
    const createdItems = await pack.createEmbeddedDocuments('Item', createData);
    console.log(`BBTTCC | Created ${createdItems.length} Character Archetypes`);
}

async function importCrewTypes() {
    const pack = game.packs.get('bbttcc-character-options.crew-types');
    if (!pack) throw new Error('Crew types pack not found');
    
    console.log('BBTTCC | Importing Crew Types...');
    ...
    const createData = crewTypes.map(item => ({ ...item, folder: null, sort: 0 }));
    const createdItems = await pack.createEmbeddedDocuments('Item', createData);
    console.log(`BBTTCC | Created ${createdItems.length} Crew Types`);
}