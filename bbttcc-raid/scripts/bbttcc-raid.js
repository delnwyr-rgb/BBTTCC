...
class BBTTCCRaidModule {
    ...
    static MODULE_TITLE = 'BBTTCC Raid v4.8.1-ENHANCED';
    ...
    static exposeAPI() {
        try {
            ...
            console.log(`${this.MODULE_TITLE} | Exposing API with available classes:`, {
                RaidPlanner: typeof RaidPlanner !== 'undefined'
            });
            ...
            if (module) {
                module.api = {
                    ...
                    ...(typeof RaidPlanner !== 'undefined' ? { RaidPlanner } : {}),
...
Hooks.once('init', async () => {
    console.log('BBTTCC Raid v4.8.1-ENHANCED | Starting initialization with FoundryVTT v13+ and D&D5e v5.4+ support...');
    
    // D&D5e v5.4 compatibility note
    if (game.system.id === 'dnd5e') {
        const version = game.system.version;
        if (foundry.utils.isNewerVersion(version, "5.4.0")) {
            console.log(`bbttcc-raid | Running on D&D5e v${version} - fully compatible`);
        } else if (foundry.utils.isNewerVersion(version, "5.0.0")) {
            console.warn(`bbttcc-raid | Running on D&D5e v${version} - compatible but may show deprecation warnings. v5.4+ recommended.`);
        }
    }
    
    // Register settings first per FoundryVTT best practices
    BBTTCCRaidModule.registerSettings();
    
    await BBTTCCRaidModule.initialize();
});

console.log('BBTTCC Raid v4.8.1-ENHANCED | Module script loaded');
