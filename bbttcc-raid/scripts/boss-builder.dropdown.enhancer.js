...
(() => {
  ...
  function normalizeStats(stats){
    ...
    return {
      violence: num(src.violence, 0),
      intrigue: num(src.intrigue, 0),
      softpower: num(src.softpower, num(src.softPower, 0))
    };
  }
  ...
  function renderEditors(app){
    ...
    if (!statsBox) {
      ...
      statsBox.style.gridTemplateColumns = 'repeat(3, minmax(110px, 1fr))';
    ...
    statsBox.innerHTML =
      "<label><div>Violence</div><input type='number' data-boss-stat='violence' value='" + esc(String(ns.violence)) + "'/></label>" +
      "<label><div>Intrigue</div><input type='number' data-boss-stat='intrigue' value='" + esc(String(ns.intrigue)) + "'/></label>" +
      "<label><div>Soft Power</div><input type='number' data-boss-stat='softpower' value='" + esc(String(ns.softpower)) + "'/></label>";
    ...
    if (picked.length) {
      ...
      for (let i=0; i<picked.length; i++) {
        const key = picked[i];
        const hit = mans.find(function(row){ return String(row.key || "") === String(key || ""); }) || null;
