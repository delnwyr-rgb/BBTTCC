/**
 * BBTTCC Raid Creator Macro - v4.8.1-ENHANCED
                    ...
                    create: {
                        ...
                        callback: async (html) => {
                            try {
                                ...
                                const raidName = element.querySelector('#raid-name')?.value?.trim();
                                const raidType = element.querySelector('#raid-type')?.value;
                                const raidDifficulty = element.querySelector('#raid-difficulty')?.value;
                                const raidTarget = element.querySelector('#raid-target')?.value?.trim();
                                const raidDescription = element.querySelector('#raid-description')?.value?.trim();
                                const raidObjectives = element.querySelector('#raid-objectives')?.value?.trim();
                                const selectedParticipants = element.querySelector('#raid-participants')?.value || [];
                                
                                /* Resource allocation */
                                const resources = {
                                    violence: parseInt(element.querySelector('#resource-violence')?.value) || 0,
                                    nonLethal: parseInt(element.querySelector('#resource-nonlethal')?.value) || 0,
                                    intrigue: parseInt(element.querySelector('#resource-intrigue')?.value) || 0,
                                    economy: parseInt(element.querySelector('#resource-economy')?.value) || 0
                                };
