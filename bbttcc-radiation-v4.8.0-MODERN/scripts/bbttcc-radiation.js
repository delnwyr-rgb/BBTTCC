...
class BBTTCCRadiationModule {
    ...
    static MODULE_TITLE = 'BBTTCC Radiation v4.8.0 - MODERN';
    ...
    static async onReady() {
        try {
            ...
            if (!game.system.id === 'dnd5e') {
                throw new Error('BBTTCC Radiation requires D&D 5e system');
            }
        ...
        try {
            ...
            if (module) {
                module.api = {
                    ...
                    createRadiationZone: this.createRadiationZone.bind(this),
    ...
    static onRenderTokenHUD(app, html, data) {
        ...
        try {
            ...
            const button = `
                <div class="control-icon" data-action="radiation" title="Radiation: ${radiationLevel.name} (${radiationData.level.toFixed(1)}%)">
                    <i class="fas fa-radiation" style="color: ${radiationLevel.color}"></i>
                </div>
            `;
            
            html.find('.right').append(button);
            
            html.find('[data-action="radiation"]').click(() => {
...
console.log('BBTTCC Radiation v4.8.0 - MODERN | Module script loaded');
