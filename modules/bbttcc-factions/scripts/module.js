...
function isCharacter(a) {
  // 2026-05-13 — include NPCs as roster-eligible. NPCs can belong to a
  // faction (e.g., authoring a bad-guy faction with NPC-only members) and
  // should appear in the Roster + Relationships sections.
  const t = a?.type ?? foundry.utils.getProperty(a, "type");
...
function _bbttccBuildDoctrineDescription(kind, key, spec) {
  const label = spec?.label || _bbttccDoctrineLabelForKey(kind, key);
  const rawText = spec?.text ? String(spec.text) : "";
  const bodyHtml = /<[a-z][\s\S]*>/i.test(rawText)
    ? rawText
    : (rawText ? `<p>${_bbttccEscapeHtml(rawText)}</p>` : `<p><em>No rules text registered for this ${kind}.</em></p>`);
  const costs = spec?.opCosts || {};
  const costPairs = Object.entries(costs).filter(([, v]) => Number(v));
  const costsHtml = costPairs.length
    ? `<p><strong>OP Cost:</strong> ${costPairs.map(([k, v]) => `${_bbttccEscapeHtml(k)} ${v}`).join(", ")}</p>`
    : "";
  const tierHtml = spec?.tier != null ? `<p><strong>Tier:</strong> ${_bbttccEscapeHtml(spec.tier)}</p>` : "";
  const rarityHtml = spec?.rarity ? `<p><strong>Rarity:</strong> ${_bbttccEscapeHtml(spec.rarity)}</p>` : "";
  const raidTypesHtml = Array.isArray(spec?.raidTypes) && spec.raidTypes.length
    ? `<p><strong>Raid Types:</strong> ${spec.raidTypes.map(_bbttccEscapeHtml).join(", ")}</p>`
    : "";
  const defenderHtml = spec?.defenderAccess
    ? `<p><strong>Defender Access:</strong> ${_bbttccEscapeHtml(spec.defenderAccess)}</p>`
    : "";
  return `<div class="bbttcc-doctrine-desc"><h3>${_bbttccEscapeHtml(label)}</h3>${bodyHtml}${costsHtml}${tierHtml}${rarityHtml}${raidTypesHtml}${defenderHtml}</div>`;
}
...
async function effHexWithAll(dr) {
  ...
  return { ...eff, defenseBonus: defense };
}
...
function _characterBelongsToFaction(char, faction) {
  // 2026-05-13 — also honor the fourththing-native `system.faction.id`
  // path that the NPC + character sheets' faction dropdowns write to.
  // Without this, NPCs whose faction is set via the sheet dropdown don't
  // appear in the Roster section because they only have the system field
  // populated, not the bbttcc-factions flag.
...
function _zeros() {
  return { food:0, materials:0, trade:0, military:0, knowledge:0, defense:0 };
}
...
function fmtResLine(obj = {}) {
  ...
  return `F ${n(obj.food)} • M ${n(obj.materials)} • T ${n(obj.trade)} • Mil ${n(obj.military)} • K ${n(obj.knowledge)} • Def ${n(obj.defense)}`;
}
function _zerosOP() {
...
const RIG_PRESETS = {
  ...
  "war-rig": {
    ...
    combat: { role: "combat", power: 3, capMods: { violence: 20 }, systems: ["armor_plating"], signature: "breakthrough" },
    passiveBonuses: [
  ...
  "support-rig": {
    ...
    combat: { role: "support", power: 2, capMods: { logistics: 20, economy: 10 }, systems: ["repair_teams"], signature: "patch_and_push" },
    passiveBonuses: [
      { key: "quartermaster_train", label: "Quartermaster Train", kind: "travel", op: { economy: -10 }, hazardChance: -0.05, notes: "Improves supply efficiency and route planning." }
    ]
  },
  "scout-rig": {
    name: "Scout Rig",
    ...
    combat: { role: "scout", power: 2, capMods: { intrigue: 20, logistics: 10 }, systems: ["route_scanner"], signature: "hit_and_vanish" },
    passiveBonuses: [
  ...
  "jackalope-sprinter": {
    ...
    combat: { role: "scout", power: 2, capMods: { logistics: 20, intrigue: 10 }, systems: ["route_scanner", "signal_mask"], signature: "hit_and_vanish" },
    passiveBonuses: [
      { key: "forward_scouts", label: "Forward Scouts", kind: "travel", hazardChance: -0.10, encounterTierBias: { down: 1 }, notes: "Spots danger early; nudges encounters downward." },
  ...
  "needlewing-courier-skiff": {
    ...
    combat: { role: "scout", power: 2, capMods: { intrigue: 20, diplomacy: 10 }, systems: ["signal_mask"], signature: "black_bag_drop" },
    passiveBonuses: [
  ...
  "attaccountant-auditor-platform": {
    ...
    combat: { role: "support", power: 3, capMods: { economy: 30, intrigue: 20, diplomacy: 10 }, systems: ["abacus_array", "compliance_bulwark"], signature: "ledger_lock" },
    passiveBonuses: [
      { key: "quartermaster_train", label: "Quartermaster Train", kind: "travel", op: { economy: -10 }, notes: "Audit-driven supply efficiency; trims economy spend." },
  ...
  "field-workshop-crawler": {
    ...
    combat: { role: "support", power: 2, capMods: { logistics: 30, economy: 10 }, systems: ["spare_parts_vault"], signature: "patch_and_push" },
    passiveBonuses: [
      { key: "quartermaster_train", label: "Quartermaster Train", kind: "travel", op: { economy: -10 }, notes: "Parts discipline reduces waste and improves routing." },
  ...
  "chorus-relay-shrine": {
    ...
    combat: { role: "support", power: 3, capMods: { softpower: 30, faith: 20 }, systems: ["choir_antennae"], signature: "harmony_pulse" },
    passiveBonuses: [
  ...
  "burrow-blaster": {
    ...
    combat: { role: "combat", power: 4, capMods: { violence: 30, logistics: 10, intrigue: 10 }, systems: ["seismic_drill", "tremor_mortar"], signature: "subterranean_shock" },
    passiveBonuses: [
  ...
  "red-herring-chassis": {
    ...
    combat: { role: "combat", power: 4, capMods: { violence: 20, intrigue: 20 }, systems: ["spike_ram"], signature: "breakthrough" },
    passiveBonuses: [
      { key: "armored_screen", label: "Armored Screen", kind: "travel", travelDefense: 1, notes: "Reduces losses from ambushes while traveling." }
    ]
  },
  "knotsteel-longboat-tank": {
    ...
    combat: { role: "combat", power: 3, capMods: { violence: 30, diplomacy: 10 }, systems: ["knot_glyph_plating"], signature: "boarding_party" },
    passiveBonuses: [
  ...
  "oracle-spindle-war-tower": {
    ...
    combat: { role: "command", power: 5, capMods: { intrigue: 30, diplomacy: 30, logistics: 20 }, systems: ["field_tribunal", "signal_web"], signature: "command_override" },
    passiveBonuses: [
      { key: "quartermaster_train", label: "Signal Web Logistics", kind: "travel", op: { logistics: -10 }, notes: "Command routing reduces logistics spend (Phase 2 travel bridge)." },
...
const TURN_EFFECT_PRESETS = {
  ...
  "supply_cache": { key: "supply_cache", economy: 10, notes: "Adds +1 Economy OP during turn regen." },
...
class BBTTCCFactionSheet extends ActorSheet {
  ...
  // open faction sheets so the "GM Manual Edit" fieldset shows/hides without
  // a settings menu round-trip. GM-only; quietly omitted for players.
  //
  // NOTE: `class` MUST be a single token. Foundry V13's V1 header-button
  // dispatcher (application-v1.mjs:558-562) uses `classList.contains(b.class)`
  // to map clicks back to the button object — a multi-class string like
  // "foo is-on" can never match, so the onclick silently never fires.
  // ON/OFF state lives in the icon + label, NOT the class.
  _getHeaderButtons() {
    const buttons = super._getHeaderButtons();
    try {
      if (game.user?.isGM) {
        const enabled = !!game.settings?.get?.("bbttcc-core", "gmEditMode");
        buttons.unshift({
          label:   enabled ? "Edit: ON" : "Edit: OFF",
          class:   "bbttcc-gm-edit-toggle",
          icon:    enabled ? "fas fa-unlock" : "fas fa-lock",
          onclick: async () => {
            try {
              const next = !game.settings.get("bbttcc-core", "gmEditMode");
              await game.settings.set("bbttcc-core", "gmEditMode", next);
              const newLabel = next ? "Edit: ON" : "Edit: OFF";
              const newIcon  = next ? "fas fa-unlock" : "fas fa-lock";
              for (const app of Object.values(ui.windows ?? {})) {
                if (!(app instanceof BBTTCCFactionSheet)) continue;
                app.render(false);
                // Header buttons live in the window frame, NOT the form body,
                // so _renderInner doesn't refresh them. Patch the DOM directly
                // so label/icon stay in sync with the actual setting state.
                const $btn = app.element?.find?.(".header-button.bbttcc-gm-edit-toggle");
                if ($btn?.length) $btn.html(`<i class="${newIcon}"></i> ${newLabel}`);
              }
            } catch (e) {
              console.warn("[bbttcc-factions] GM edit toggle failed", e);
              ui.notifications?.warn("Could not toggle GM Edit Mode (is bbttcc-core loaded?).");
            }
          }
        });
      }
    } catch (_e) { /* defensive — keep base buttons even if probing fails */ }
    return buttons;
  }
...
const politicalPressure = {
  ...
  driftScore,
  driftPct: Math.max(0, Math.min(100, Math.round(((driftScore + 100) / 200) * 100))),
  severityState,
  lastImpacts
};
    ...
    const loyalty  = Math.max(0, Number(this.actor.getFlag(MODULE_ID, "loyalty") ?? 0) || 0);
    const momentum = Math.max(0, Number(this.actor.getFlag(MODULE_ID, "momentum") ?? 0) || 0);
    const bonusDieArmed = !!this.actor.getFlag(MODULE_ID, "nextRollBonusDie");
    // Darkness canon: flags.bbttcc-factions.darkness = { global, [hexId]: N, ... }.
    // Tolerate legacy flat-number storage so worlds upgraded mid-stream still display.
    const darknessRaw = this.actor.getFlag(MODULE_ID, "darkness");
    const darknessGlobal = (darknessRaw && typeof darknessRaw === "object" && !Array.isArray(darknessRaw))
      ? darknessRaw.global
      : darknessRaw;
    const darknessValue = Math.max(0, Number(darknessGlobal ?? 0) || 0);
    const victoryFlags = foundry.utils.duplicate(this.actor.getFlag(MODULE_ID, "victory") || {});
    const victory = {
      ...victoryFlags,
      vp: Math.max(0, Number(victoryFlags?.vp ?? 0) || 0),
      unity: Math.max(0, Number(victoryFlags?.unity ?? 0) || 0)
    };
...
try {
  ...
  if (rootEl && rootEl instanceof HTMLElement && gmEnabled && !rootEl.dataset.bbttccGmEditBound) {
    ...
    rootEl.addEventListener("click", async (ev) => {
      ...
      // Darkness must stay in object shape `{ global, [hexId]: N, ... }` — travel/raid
      // readers (hex-travel-mode darknessBump, victory enhancer) only look at .global
      // and per-hex keys. Writing a flat number here clobbers the schema and silently
      // zeroes downstream darkness pressure.
      const existingDarkness = this.actor.getFlag(MODULE_ID, "darkness");
      const newDarkness = (existingDarkness && typeof existingDarkness === "object" && !Array.isArray(existingDarkness))
        ? foundry.utils.duplicate(existingDarkness)
        : {};
      newDarkness.global = darkness;
...
// --- /AAE Politics controls ---
  ...
  _bindRollButtons(root) {
    ...
    // Momentum Spend button (may appear once inside the OP card footer)
    const spendBtn = root.querySelector?.("[data-bbttcc-momentum-spend]");
    if (spendBtn && spendBtn.dataset.bbttccBound !== "1") {
      spendBtn.dataset.bbttccBound = "1";
      spendBtn.addEventListener("click", (ev) => {
        ev.preventDefault();
        try { this._onMomentumSpend(); }
        catch (e) { console.error(`[${MODULE_ID}] momentum spend failed`, e); }
      });
    }
    ...
    for (const btn of buttons) {
      ...
      btn.addEventListener("click", async (ev) => {
        ...
        try {
          ...
          // RFI strategic layer: d10s explode on 10, each extra die banks +1 Momentum.
          // Momentum spend "Add +1d10" sets a one-shot flag that grows the pool to 3.
          const bonusDieArmed = !!actor.getFlag(MODULE_ID, "nextRollBonusDie");
          if (bonusDieArmed) await actor.unsetFlag(MODULE_ID, "nextRollBonusDie");
          const poolSize = bonusDieArmed ? 3 : 2;
          const formula  = `${poolSize}d10x10 + @b`;

          const roll = new Roll(formula, { b: base + contrib });
          await roll.evaluate({ async: true });

          const allResults = roll.terms[0]?.results ?? [];
          const baseDice   = allResults.slice(0, poolSize).map(r => r.result);
          const explosions = Math.max(0, allResults.length - poolSize);
          const doubleTen  = baseDice.filter(v => v === 10).length >= 2;

          if (explosions > 0) {
            const curMomentum = Number(actor.getFlag(MODULE_ID, "momentum") ?? 0);
            await actor.setFlag(MODULE_ID, "momentum", curMomentum + explosions);
          }

          const label = key.charAt(0).toUpperCase() + key.slice(1);
          const noteBits = [];
          if (bonusDieArmed)   noteBits.push(`+1d10 spent — pool = 3d10`);
          if (explosions > 0)  noteBits.push(`+${explosions} Momentum banked`);
          if (doubleTen)       noteBits.push(`✦ Decisive Moment — GM adjudicates a free strategic beat this turn.`);
          const noteHtml = noteBits.length
            ? `<br/><small style="color:#e8c84a">${noteBits.join(" · ")}</small>`
            : "";

          roll.toMessage({
            speaker: { alias: actor.name },
            flavor: `<strong>${actor.name}</strong> — ${label} Check<br/><small>Bonus = Value (${base}) + Roster (${contrib})</small>${noteHtml}`
          });
  ...
  async _onMomentumSpend() {
    const actor = this.actor;
    const momentum = Number(actor.getFlag(MODULE_ID, "momentum") ?? 0);

    if (momentum <= 0) {
      return ui.notifications?.warn?.(`${actor.name}: no Momentum banked to spend.`);
    }

    const armed = !!actor.getFlag(MODULE_ID, "nextRollBonusDie");

    const OPTIONS = [
      { cost: 1, key: "bonusDie",  label: "Add +1d10 exploding to this faction's next OP check.",
        disabledReason: armed ? "Already armed" : null },
      { cost: 2, key: "reroll",    label: "Force a reroll of an opposed strategic check." },
      { cost: 3, key: "refund",    label: "Refund one OP cost on the current turn (GM adjudicates)." },
      { cost: 5, key: "reshape",   label: "Reshape Strategy — narrative reshaping of a strategic-layer event (GM-gated)." }
    ];

    const optHtml = OPTIONS.map(o => {
      const blocked = (momentum < o.cost) || !!o.disabledReason;
      const reason  = o.disabledReason ? ` <em style="opacity:.7">(${o.disabledReason})</em>` : "";
      return `
        <button type="button" class="bbttcc-momentum-opt" data-cost="${o.cost}" data-key="${o.key}"
                ${blocked ? "disabled" : ""}
                style="display:block;width:100%;text-align:left;margin:.3rem 0;padding:.45rem .6rem;
                       background:${blocked ? "rgba(255,255,255,0.03)" : "rgba(232,200,74,0.1)"};
                       border:1px solid ${blocked ? "rgba(255,255,255,0.1)" : "rgba(232,200,74,0.4)"};
                       border-radius:4px;color:${blocked ? "rgba(255,255,255,0.3)" : "#e8c84a"};
                       cursor:${blocked ? "default" : "pointer"}">
          <div style="font-weight:600;font-size:0.82rem">${o.cost} Momentum${reason}</div>
          <div style="font-size:0.72rem;opacity:0.85;margin-top:.15rem">${o.label}</div>
        </button>`;
    }).join("");

    const content = `
      <div class="bbttcc-momentum-spend">
        <p style="margin:0 0 .5rem;font-size:0.82rem">
          Banked: <b style="color:#e8c84a">${momentum} Momentum</b>. Choose one:
        </p>
        ${optHtml}
      </div>`;

    const dialog = new Dialog({
      title: `Spend Momentum — ${actor.name}`,
      content,
      buttons: { close: { label: "Close" } },
      default: "close",
      render: (html) => {
        const $html = html.jquery ? html : $(html);
        $html.find(".bbttcc-momentum-opt").on("click", async (e) => {
          const btn  = e.currentTarget;
          if (btn.disabled) return;
          const cost = Number(btn.dataset.cost);
          const key  = btn.dataset.key;
          const cur  = Number(actor.getFlag(MODULE_ID, "momentum") ?? 0);
          if (cur < cost) return ui.notifications?.warn?.("Not enough Momentum.");

          await actor.setFlag(MODULE_ID, "momentum", cur - cost);

          let flavor = "";
          if (key === "bonusDie") {
            await actor.setFlag(MODULE_ID, "nextRollBonusDie", true);
            flavor = `<strong>${actor.name}</strong> spends ${cost} Momentum — <em>+1d10 armed</em> for the next OP check.`;
          } else if (key === "reroll") {
            flavor = `<strong>${actor.name}</strong> spends ${cost} Momentum — <em>forces a reroll</em> on the current opposed strategic check. GM adjudicates.`;
          } else if (key === "refund") {
            flavor = `<strong>${actor.name}</strong> spends ${cost} Momentum — <em>OP refund</em> requested on the current turn. GM adjudicates.`;
          } else if (key === "reshape") {
            flavor = `<strong>${actor.name}</strong> spends ${cost} Momentum — <em>Reshape Strategy</em>. GM adjudicates a narrative reshape this turn.`;
          }

          ChatMessage.create({
            speaker: { alias: actor.name },
            content: `<div class="bbttcc-momentum-card" style="padding:.3rem">
              <div style="color:#e8c84a;font-weight:600">✦ Momentum Spent</div>
              <div style="font-size:0.82rem">${flavor}</div>
              <div style="font-size:0.7rem;opacity:0.7;margin-top:.2rem">Momentum: ${cur} → ${cur - cost}</div>
            </div>`
          });
...
Hooks.once("init", () => {
  ...
  try {
    if (globalThis.Handlebars) {
      ...
      // Tooltip lookups for the faction sheet — single source of truth.
      // Mechanics-first, one fiction beat. Used by bbttcc_opTip / bbttcc_factionTip.
      const BBTTCC_OP_TIPS = {
        violence:   "Violence — open force. Soldiers, militia, raid bands. Spend on assault, sabotage-by-force, holding ground.",
        nonlethal:  "Nonlethal — pressure short of killing. Riot lines, blockades, crowd control. Wins influence without escalating to body count.",
        intrigue:   "Intrigue — spies, sleights, leverage. Espionage rolls, opposed counter-intel, blackmail. Pairs with Steward Stealth/Insight.",
        economy:    "Economy — coin, trade, supply chain. Funds operations, fuels Logistics, sets ceiling on what other tracks can spend.",
        softpower:  "Soft Power — propaganda, narrative, mass-attention. Sways uncommitted populations, seeds Political Pressure drift.",
        diplomacy:  "Diplomacy — formal channels, treaties, envoys. Opens alliances, calls in favors, negotiates ceasefires.",
        logistics:  "Logistics — supply, movement, infrastructure. Lets you project the OTHER tracks at distance; weak Logistics caps your reach.",
        culture:    "Culture — identity, art, ritual. Slow to build, hardest to crush; long-arc Unity and Loyalty growth.",
      ...
      const BBTTCC_FACTION_TIPS = {
        ...
        loyalty:    "Loyalty — adherence to faction leadership (0–100%). Low Loyalty risks defections, sabotage from within, Pressure flips.",
        darkness:   "Darkness — accumulated Qliphothic taint. Rises from atrocity, ritual desecration, sephirotic neglect. High Darkness corrupts rolls and recruits.",
        momentum:   "Momentum — strategic-layer Surge. Banked +1 each 10 rolled on OP checks; resets each turn. Spend menu: 1=arm +1d10 next OP roll · 2=force opposed reroll · 3=refund OP cost · 5=Reshape Strategy (GM-gated).",
        bonusDieArmed: "Next OP roll fires 3d10 (still exploding). Auto-consumed when used.",
      ...
      if (!Handlebars.helpers.bbttcc_opTip) {
        Handlebars.registerHelper("bbttcc_opTip", k => BBTTCC_OP_TIPS[k] ?? "");
      }
...
Hooks.on("bbttcc:advanceTurn:end", async () => {
  try {
    ...
    for (const a of (game.actors?.contents ?? [])) {
      ...
      const cur = Number(a.getFlag(MODULE_ID, "momentum") ?? 0);
      if (cur > 0) await a.setFlag(MODULE_ID, "momentum", 0);
      if (a.getFlag(MODULE_ID, "nextRollBonusDie")) {
        await a.unsetFlag(MODULE_ID, "nextRollBonusDie");
      }
    }
  } catch (e) { warn("momentum turn-reset failed", e); }
});
...
Hooks.once("ready", async () => {
  try {
    ...
    const PKG_STANDARD_V1 = {
      ...
      // OP economy (bank actual + bank potential), in MARKS (1 OP = 10 marks).
      opCaps: _opObj({
        violence: 50,
        nonlethal: 50,
        intrigue: 50,
...
factionApi.applyStartingPackage ??= (async ({
  ...
  const patch = {
    ...
    // 2026-05-13 — Starting-package opSeed/opCaps are already in MARKS
    // (1 OP = 10 marks). Stamp the marks-migration flag so the one-shot
    // `opBankMarksMigrated` sweep in op-engine.js does NOT re-multiply
    // these values × 10 on next world load. Without this flag, fresh
    // factions get a 10× bank inflation right after creation.
...
factionApi.openCreationWizard ??= (() => {
  return async () => {
    ...
    const name = await new Promise((resolve) => {
      new Dialog({
        ...
        buttons: {
          ...
          create: {
            icon: '<i class="fas fa-flag"></i>', label: "Create",
            callback: (html) => {
    ...
    try {
      ...
      ui.notifications.info(`Faction created: ${name}`);
...
// Auto-refresh any open faction sheet when its bbttcc-factions flags change.
// Without this, the sheet keeps showing the opBank / morale / loyalty / etc.
// it read at render time — so a travel that debits OP, a raid that shifts
// morale, or Advance Turn replenishing the bank all leave the sheet stale
// until it's closed and reopened. We only re-render on OUR flag changes and
// skip if no sheet is actually rendered, so this is cheap.
Hooks.on("updateActor", (actor, data) => {
  try {
    if (!foundry.utils.hasProperty(data, `flags.${MODULE_ID}`)) return;
    const sheet = actor.sheet;
    if (!sheet || !sheet.rendered) return;
    sheet.render(false);
  } catch (e) { warn("faction-sheet auto-refresh hook", e); }
});
...
Hooks.once("ready", () => {
  ...
  setTimeout(async () => {
    try {
      const EFFECTS = game.bbttcc?.api?.raid?.EFFECTS || {};
      const actors = Array.from(game.actors ?? []).filter(a => isFactionActor(a));
      let touched = 0;
      for (const a of actors) {
        const items = Array.from(a.items ?? []).filter(it => _bbttccIsDoctrineItem(it));
        const updates = [];
        for (const it of items) {
          const existing = String(it.system?.description?.value ?? "").trim();
          if (existing) continue;
          const kind = String(it.flags?.bbttcc?.kind || "").toLowerCase();
          const key  = String(it.flags?.bbttcc?.key  || "").toLowerCase();
          if (!kind || !key) continue;
          const spec = EFFECTS[key] || null;
          const html = _bbttccBuildDoctrineDescription(kind, key, spec);
          updates.push({ _id: it.id, system: { description: { value: html, chat: "" } } });
        }
        if (updates.length) {
      ...
      if (touched) log(`doctrine description backfill — updated ${touched} item(s)`);
    } catch (e) {
      warn("doctrine description backfill failed", e);
    }
  }, 1500);
});
