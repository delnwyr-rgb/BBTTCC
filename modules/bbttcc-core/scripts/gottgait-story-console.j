...
(function () {
  ...
  var TAG_GROUPS = [
    { id:"tag-travel-threshold", tag:"trigger.travel_threshold", label:"Campaign Beats — Travel Threshold", description:"Beats tagged for travel-threshold injection." },
    { id:"tag-hidden-vault",     tag:"hv.hidden_vault",         label:"Campaign Beats — Hidden Vault Thread", description:"Vault thread beats authored in the Campaign Builder." },
    { id:"tag-enforcement",      tag:"enforcement",            label:"Campaign Beats — Enforcement / Convergence", description:"Escalation and enforcement beats." },
    { id:"tag-debt",             tag:"debt",                   label:"Campaign Beats — Debt Pressure", description:"Debt-ish beats." },
    { id:"tag-gilbert",          tag:"gilbert",                label:"Campaign Beats — Gilbert / Auditor", description:"Thread E / auditor pressure beats." }
  ];
  ...
  function promptInjectorFire(opts) {
    ...
    var tags = defaultTags || last.tags || "trigger.travel_threshold";
  ...
  class GOTTGAITStoryConsole extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    async _prepareContext() {
      ...
      var gmGroups = [{
        id: "gm-advisor",
        label: "GM Advisor — World Signals",
                ...
                if (beat === "gm.advisor.refresh") {
                  ...
                  Promise.resolve(gmApi.recommendWorldSignals()).then(function (res) {
                    ...
                    var content = ''
                      ...
                      + '<p><strong>Suggested Beats</strong></p>'
                      + '<ul>' + (beatItems || '<li>No immediate beat suggestion available from the active campaign.</li>') + '</ul>'
                      + '</div>';
                    ...
                    for (var ti = 0; ti < tableSuggestions.length; ti++) {
                      (function (sugg, idx) {
                        ...
                        buttons['table' + idx] = {
                          label: 'Run Table: ' + String(sugg.label || sugg.tableId || ('Table ' + (idx + 1))),
                          callback: function () {
                    ...
                    new Dialog({
                      title: "GM Advisor — World Signals",
                      content: content,
                      buttons: buttons,
  ...
  Hooks.once("ready", function () {
    ...
    console.log(TAG, "GOTTGAIT Story Console ready.");
  });
