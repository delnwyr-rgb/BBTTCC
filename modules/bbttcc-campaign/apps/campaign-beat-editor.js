...
const BEATS_HELP = {
  ...
  timeScale: "Time Scale — narrative size only. Beats no longer spend days (owner ruling 2026-09-13: the turn is a chapter break; the Day chip is a prompt). Kept for authoring flavour.",
...
export class BBTTCCCampaignBeatEditorApp extends Application {
  ...
  constructor(options = {}) {
    ...
    // When set, the bbttcc-tikkun beat-listener routes the beat resolution
    // into the spark API. See:
    //   modules/bbttcc-tikkun/scripts/tikkun-beat-listener.enhancer.js
    //   project_tikkun_revisit_plan_2026_05_09.md
    this.beat.sparkLink ??= {
      sparkUuid: "",
      action:    "",
      methodTag: ""
  ...
  async getData() {
    ...
    // Spark Link options (Phase B of B3 — beat-driven Tikkun progression).
    // Loaded from the bbttcc-tikkun.sparks compendium pack so authors get
    // the live list of authored sparks. Falls back to (None) if the pack
    // isn't installed.
    ...
    try {
      ...
      if (sparkPack) {
        ...
        const entries = idx
          ...
          .sort((a, b) => a.label.localeCompare(b.label));
        if (entries.length) sparkLinkOptions = sparkLinkOptions.concat(entries);
      }
    } catch (e) { /* no pack, no link */ }

    const sparkLinkActions = [
      { id: "",          label: "— No action —" },
      { id: "identify",  label: "Identify (mark known)"          },
      { id: "acquire",   label: "Acquire / Gather (gain)"        },
      { id: "integrate", label: "Integrate (absorb personally)"  },
      { id: "deposit",   label: "Deposit to Faction (Phase E)"   }
    ];

    // Method tags for Phase C corruption detection. Mirrors OP-pool keys.
    const sparkLinkMethods = [
      { id: "",          label: "— Inherit from ctx —" },
      { id: "violence",  label: "Violence" },
      { id: "intrigue",  label: "Intrigue" },
      { id: "diplomacy", label: "Diplomacy" },
      { id: "softpower", label: "Soft Power" },
      { id: "nonlethal", label: "Non-lethal" },
    ...
    const sparkLink = foundry.utils.deepClone(this.beat.sparkLink ?? { sparkUuid: "", action: "", methodTag: "" });

    // Gate Unlocks — enumerate every leyline-aware hex Drawing across all
    // scenes so the GM can pick anchors by name. The listener
    // (bbttcc-territory/scripts/bbttcc-gate-beat-listener.js) consumes
    // `beat.gateUnlocks: [{hexUuid, action}]` on bbttcc:beat:resolved.
    const gateUnlocksCurrent = Array.isArray(this.beat.gateUnlocks) ? this.beat.gateUnlocks : [];
    const gateUnlockSelectedSet = new Set(gateUnlocksCurrent.map(g => String(g?.hexUuid || "").trim()).filter(Boolean));
    const gateUnlockSelectedAction = (gateUnlocksCurrent.find(g => g?.action) ?? {}).action || "enable";

    let gateUnlockHexOptions = [];
    try {
      for (const sc of (game.scenes ?? [])) {
        for (const drDoc of (sc.drawings ?? [])) {
          ...
          const uuid = `Scene.${sc.id}.Drawing.${drDoc.id}`;
          gateUnlockHexOptions.push({
            uuid,
            label: `${sc.name || sc.id} — ${nm}`,
            selected: gateUnlockSelectedSet.has(uuid)
          });
        }
      }
      gateUnlockHexOptions.sort((a, b) => a.label.localeCompare(b.label));
      // Surface already-selected entries that point to deleted/cross-world
      // hexes so they don't silently drop on save.
      const present = new Set(gateUnlockHexOptions.map(o => o.uuid));
      for (const uuid of gateUnlockSelectedSet) {
        if (!present.has(uuid)) {
          gateUnlockHexOptions.unshift({ uuid, label: `⚠ Orphan link: ${uuid}`, selected: true });
        }
      }
    } catch (e) {
      gateUnlockHexOptions = [];
    }

    const gateUnlockActions = [
      { id: "enable",  label: "Enable (open gate)"  },
      { id: "disable", label: "Disable (close gate)" }
    ];
...
_ensureWorldModifiersUI(html) {
  ...
  try {
    ...
    this.beat.audio ??= { enabled: false, src: "", playlistSoundUuid: "", volume: 0.85, loop: false, autoplay: false, broadcastPlayers: false };
    ...
    // Build the Playlist Sound picker options. Grouped by parent Playlist.
    // Selecting a PlaylistSound routes playback through Foundry's native
    // PlaylistSound.update({playing:true}) — which handles preload, player
    // broadcast, and stop via document sync. Takes precedence over raw src.
    const selectedPlaylistUuid = String(aud.playlistSoundUuid || "").trim();
    const playlistOptionsHtml = (() => {
      try {
        const parts = [`<option value="">— None (use raw file path below) —</option>`];
        const playlists = (game?.playlists?.contents || []).slice().sort(
          (a, b) => String(a.name || "").localeCompare(String(b.name || ""))
        );
        for (const pl of playlists) {
          const sounds = (pl.sounds?.contents || []).slice().sort(
            (a, b) => String(a.name || "").localeCompare(String(b.name || ""))
          );
          if (!sounds.length) continue;
          parts.push(`<optgroup label="${esc(pl.name || "(unnamed playlist)")}">`);
          for (const ps of sounds) {
            const uuid = String(ps.uuid || "");
            const sel = uuid && uuid === selectedPlaylistUuid ? " selected" : "";
            parts.push(`<option value="${esc(uuid)}"${sel}>${esc(ps.name || "(unnamed sound)")}</option>`);
          }
          parts.push(`</optgroup>`);
        }
        return parts.join("");
    ...
    wrap.innerHTML = `
      ...
      <div class="bbttcc-card__body">
        ...
        <div class="form-group">
          ...
          <p class="bbttcc-muted" style="margin:.25rem 0 0; opacity:.75;">
            If set, this takes precedence over the raw file path below. Playlists broadcast to players automatically.
          </p>
        </div>
    ...
    // Phase 2 preload: if this beat already has a src, warm the sound cache
    // now so the first Play doesn't eat cold-cache load latency.
    try {
      const existingSrc = String(aud.src || "").trim();
      if (existingSrc) {
        const api = game?.bbttcc?.api?.campaign?.audio;
        if (api && typeof api.preload === "function") api.preload(existingSrc);
      }
    } catch (_ePre0) {}
    ...
    const playlistEl = wrap.querySelector("select[name='audio-playlist-sound-uuid']");
    ...
    const sync = () => {
      ...
      a.playlistSoundUuid = String(playlistEl ? (playlistEl.value || "") : (a.playlistSoundUuid || "")).trim();
    ...
    // Immediate sync on Playlist Sound picker change so the selected UUID
    // sticks without requiring a full form-save.
    if (playlistEl) {
      playlistEl.addEventListener("change", () => { sync(); });
    }
        ...
        try {
          const fp = new FilePicker({
            ...
            callback: (path) => {
              ...
              try {
                const api = game?.bbttcc?.api?.campaign?.audio;
                if (api && typeof api.preload === "function" && path) api.preload(path);
              } catch (_ePre) {}
            }
          });
          fp.browse();
    ...
    if (playBtn) {
      playBtn.addEventListener("click", async (ev) => {
        ...
        try {
          stopPreview();
          // Route through the manager — keeps a single owner of audio state
          // and applies the preview-specific rules (no broadcast, dedupe still
          // structural). Falls back to direct AH.play if the manager isn't
          // up yet for some reason (module load race).
          const api = game?.bbttcc?.api?.campaign?.audio;
          if (api && typeof api.preview === "function") {
            this._audioPreviewSound = await api.preview(this.beat) || null;
          } else {
...
_syncCoreFromForm() {
  ...
  _syncChoicesFromForm() {
    ...
    try {
      ...
      const psUuid0 = fd.get("audio-playlist-sound-uuid");
      if (psUuid0 != null) a.playlistSoundUuid = String(psUuid0 || "").trim();
      ...
      a.broadcastPlayers = !!fd.get("audio-broadcast");
    } catch (_eAudSave) {
      /* no-op */
    }
