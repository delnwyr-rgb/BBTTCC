...
try {
  if (!globalThis.__bbttccHexChromeDialogHookInstalled) {
    ...
    Hooks.on("renderDialog", function (app, html) {
      ...
      try { setTimeout(function(){ __bbttccAutosizeDialog(app); }, 0); } catch (_e2) {}
    });
...
function _stopBeatAudio() {
  try {
    const s = __bbttccBeatAudioNow;
    if (s && typeof s.stop === "function") s.stop();
    else if (s && typeof s.fade === "function") s.fade(0, { duration: 250 });
  } catch (_e) {}
...
function _gmAdjudicate(title, promptHtml) {
  return new Promise(function (resolve) {
    try {
      ...
      dlg.render(true);
      try { setTimeout(function(){ __bbttccAutosizeDialog(dlg); }, 0); } catch (_e1) {}
...
function _readActorOp(actor, key) {
  const f =
    actor?.flags?.["bbttcc-character-options"]?.opBonuses ||
    actor?.flags?.bbttcc?.opBonuses ||
    actor?.flags?.["bbttcc-factions"]?.opContrib ||
    {};
  return _num(f[key], 0);
}
          ...
          ? `<div class="bbttcc-campaign-dialog-choices">
               ${choices.map((ch, i) => {
                 ...
                 const meta = _choiceHasCheck(ch)
                   ? (() => {
                       const statRaw = String(ch.checkStat || "").trim().toLowerCase();
                       const isOp = statRaw.indexOf("op.") === 0;
                       const opKey = isOp ? String(statRaw.split(".")[1] || "").trim().toLowerCase() : "";
                       const line = `<div style="opacity:0.85;font-size:12px;margin-top:4px;"><code>${_escapeHtml(ch.checkStat)}</code> vs DC <code>${_num(ch.checkDC ?? 0, 0)}</code></div>`;
    ...
    if (choices.length) {
      for (let i = 0; i < choices.length; i++) {
        ...
        buttons[`c${i}`] = {
          ...
          callback: async (html) => {
            try {
              const sel = html?.[0]?.querySelector('select[name="bbttccRosterActor"]');
              const rosterActorId = sel?.value || null;
              ...
              if (_choiceHasCheck(ch)) {
                ...
                if (_isGMAdjudicatedChoice(ch)) {
                  const dcTxt = (ch.checkDC != null && String(ch.checkDC).trim() !== "") ? String(_num(ch.checkDC ?? 0, 0)) : "";
                  const statTxt = String(ch.checkStat || "").trim() || "gm";
                  const prompt = String(ch.checkPrompt || ch.prompt || "").trim();

                  const metaLine = (statTxt || dcTxt)
                    ? `<div style="opacity:0.85;font-size:12px;margin-top:6px;"><code>${_escapeHtml(statTxt)}</code>${dcTxt ? ` vs DC <code>${_escapeHtml(dcTxt)}</code>` : ""}</div>`
                    : "";

                  const promptLine = prompt
                    ? `<div style="opacity:0.92; margin-top:6px;">${_escapeHtml(prompt).replaceAll("\n", "<br/>")}</div>`
                ...
                if (!isOp && mode !== "auto") {
                  const dcTxt = (ch.checkDC != null && String(ch.checkDC).trim() !== "") ? String(_num(ch.checkDC ?? 0, 0)) : "";
                  const statTxt = String(ch.checkStat || "").trim() || "check";
                  const prompt = String(ch.checkPrompt || ch.prompt || "").trim();
    ...
    try {
      ...
      if (playBtn && !playBtn.__bbttccBound) {
        ...
        playBtn.addEventListener("click", async (ev) => {
          ev.preventDefault();
          try { await _playBeatAudio(beat, { forceForNonGM: false }); }
          catch (_eP) {}
        });
      }
...
async function executeBeat(campaign, beat, ctx = {}) {
  ...
  // ✅ RESTORED: prompt/choices can appear for ANY non-encounter beat.
...
function _matchesTravelThreshold(beat) {
  ...
  return tags.includes("trigger.travel_threshold") || tags.includes("travel_threshold");
}

function _scoreBeat(beat, ctxTags = []) {
  const tags = _splitTags(beat?.tags);
  if (!tags.length) return 0;
  if (!_matchesTravelThreshold(beat)) return 0;

  let score = 100;
  const set = new Set(tags);
  for (const t of ctxTags) if (set.has(t)) score += 10;
  return score;
}
...
function _isDebtishBeat(beat) {
  const tags = _splitTags(beat?.tags);
  const set = new Set(tags);
  return set.has("debt") || set.has("gilbert") || set.has("thread.E") || set.has("auditor");
}
