...
if (save) {
  ...
  test("unscripted quests in the save keep their inferred next (the Sarmoung Hum and the spine have no script)", () => { const sit = m.deriveSituation({ beats: cb, firedSet: new Set(), readyOf: () => ({ ready: true, reasons: [] }), bucketOf: () => null, phase: 2, turn: 2, seqOf: b => Number(b.questStep) || 1e6, state: m.emptyState() }); assert.equal(sit.byKey.sarmoung_hum.script, null); assert.equal(sit.byKey.valhaulan_spine.script, null); assert.ok(sit.byKey.valhaulan_spine.next?.beat); });
