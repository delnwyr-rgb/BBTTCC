...
> ## 🆕 v2.0 Status — B3 sprint 2026-05-09
>
> The narrative-thin GM-driven loop documented below is now **mechanically complete**. Six phases shipped:
>
> | Phase | What |
> |---|---|
> | A | `spark` item type + 30 authored sparks (10 sephirot × 3 kinds) + `bbttcc:beat:resolved` hook |
> | B | Beat-driven progression — `beat.sparkLink = { sparkUuid, action, methodTag }` auto-fires spark transitions |
> | C | Auto-corruption — method × sephirah alignment table, `bbttcc:spark:corrupted` hook, sephirah-aware narration |
> | D | Repair workflow — Spark Repair Ledger on faction sheet, lightweight 2d10+skill ritual, `bbttcc:spark:repaired` hook |
> | E | Char↔Faction deposit bridge — explicit `deposited` state, character-sheet Deposit button, faction Pending Deposits ledger, `bbttcc:spark:deposited` hook |
> | F | Cleanup pass + this README note |
>
> **Authored beats now drive everything.** No console calls needed for the standard loop. The `bbttcc:scenarioResolved` hook mentioned below is **superseded by `bbttcc:beat:resolved`** (in `bbttcc-campaign/scripts/module.js`'s `executeBeat`). Authoring guidance: open Campaign Builder → edit beat → World Effects tab → Spark Link panel.
>
> Closing memos for full per-phase detail:
> - `project_sparks_as_built_2026_05_08.md` (B2 discovery)
> - `project_tikkun_revisit_plan_2026_05_09.md` (locked design decisions)
> - `project_tikkun_phase_{a,b,c,d,e,f}_2026_05_09.md` (per-phase shipping notes)
>
> ---
