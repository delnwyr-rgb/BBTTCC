...
(() => {
  ...
  async function bumpIntegrationForTarget(entry, cfg = {}) {
    ...
    // Hex Dossier — record strategic-activity-driven integration step.
    try {
      const recorder = game?.bbttcc?.api?.territory?.recordHexImprovement;
      if (typeof recorder === "function") {
        const activityKey = String(entry?.activity || entry?.activityKey || cfg.key || "strategic_activity");
        await recorder(doc, {
          kind: "integration_step",
          label: `Integration ${prev} → ${next} (${stageLabel})`,
          description: `Strategic activity '${activityKey}' advanced integration on this hex.`,
          source: { activity: activityKey, factionId: String(entry?.factionId || "") },
          before: { progress: prev },
          after: { progress: next, stageKey },
          reversible: false,
          dedupeKey: `strat_integ:${doc.id}:${activityKey}:${prev}->${next}`
        });
      }
    } catch (eL) { console.warn(TAG, "Dossier ledger write failed:", eL); }
