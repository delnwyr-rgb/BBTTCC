...
const OPTION_L1_SPECS = {
  ...
  infernal_bargain:       { primaryKey:"intrigue",   cost:{ intrigue:10 },   raidTypes:["infiltration","espionage","ritual"] },
  liturgical_rally:       { primaryKey:"softpower",  cost:{ softpower:10 },  raidTypes:["assault","liberation","siege","ritual"] },
  make_do_and_hold:       { primaryKey:"logistics",  cost:{ logistics:10 },  raidTypes:["assault_defense","occupation","siege"] },
...
divine_favor: {
  ...
  repair_fortifications: {
    ...
    apply: async ({ actor, entry }) => {
      ...
      // Hex Dossier — record facility repair step.
      try {
        const recorder = game?.bbttcc?.api?.territory?.recordHexImprovement;
        if (typeof recorder === "function") {
          await recorder(hex, {
            kind: "build",
            label: `Facility repaired (damage ${cur} → ${cur - 1})`,
            description: `Primary facility repaired one step.`,
            source: { activity: "repair_facility", factionId: actor?.id || "" },
            before: { damageStep: cur }, after: { damageStep: cur - 1 },
            reversible: false
          });
        }
      } catch (eL) { /* non-fatal */ }
