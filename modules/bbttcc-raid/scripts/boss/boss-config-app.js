...
(() => {
  ...
  class BossConfigApp extends RenderableBase {
    ...
    _readBossFromForm(root) {
      ...
      const image = q('input[name="boss.image"]') ? q('input[name="boss.image"]').value : "";
    ...
    _bindEvents(root) {
      ...
      for (let i=0; i<buttons.length; i++) {
        ...
        btn.addEventListener("click", function(ev) {
          ...
          if (action === "boss-image-edit") {
            try {
              const FP = (foundry && foundry.applications && foundry.applications.apps && foundry.applications.apps.FilePicker)
                ? foundry.applications.apps.FilePicker
                : (typeof window !== "undefined" ? window.FilePicker : null);
              if (!FP) { ui.notifications.warn("FilePicker not available."); return; }
              const cur = (root.querySelector('input[name="boss.image"]') || { value: "" }).value || "";
              const fp = new FP({
                type: "image",
                current: cur,
                callback: function(path) {
                  const inp = root.querySelector('input[name="boss.image"]');
                  if (inp) inp.value = String(path || "");
                  const prev = root.querySelector('[data-role="boss-image-preview"]');
                  if (prev) {
                    prev.innerHTML = path
                      ? '<img src="' + String(path).replace(/"/g,'&quot;') + '" alt="boss image"/>'
                      : '<div class="bbttcc-boss-art-empty">+ Add Boss Artwork</div>';
                  }
                  self._dirty = true;
                }
              });
              fp.render(true);
            } catch (e) { warn("boss-image-edit failed", e); }
            return;
          }
