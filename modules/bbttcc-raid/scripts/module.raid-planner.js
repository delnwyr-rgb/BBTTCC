...
if ((this._lockedFactionId || this._lockFaction) && !game.user.isGM) {
  ...
  facSel.title = "Faction context is locked for this window.";
      ...
      // Preserve the user's hex pick across re-renders (e.g. when they click an
      // activity row, which calls render(false)). Without this, the <select> is
      // rebuilt fresh and snaps back to its first option, causing "Plan" to
      // silently target the wrong hex.
      {
        const desiredUuid = String(this._plannerState.selectedHexUuid || "");
        const optionUuids = hexes.map(h => String(h.uuid));
        if (desiredUuid && optionUuids.includes(desiredUuid)) {
          hexSel.value = desiredUuid;
        } else if (optionUuids.length) {
          hexSel.value = optionUuids[0];
          this._plannerState.selectedHexUuid = optionUuids[0];
        } else {
          this._plannerState.selectedHexUuid = "";
        }
      }
      hexSel.addEventListener("change", () => {
        this._plannerState.selectedHexUuid = hexSel.value || "";
      });
