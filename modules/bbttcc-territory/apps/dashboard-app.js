...
export class BBTTCC_TerritoryDashboard extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
  ...
  _applyFilters(root) {
    const f = this._bbttccFilters || {};
    const nameQ = (f.name || "").trim().toLowerCase();
    const tbody = root?.querySelector?.("tbody");
    if (!tbody) return;

    let total = 0, visible = 0;
    for (const tr of tbody.querySelectorAll("tr.bbttcc-hex-row")) {
      total++;
      const d = tr.dataset;
      let show = true;
      if (nameQ && !String(d.name || "").toLowerCase().includes(nameQ)) show = false;
      if (show && f.owner) {
        if (f.owner === "__unclaimed__") { if (d.owner) show = false; }
        else if (d.owner !== f.owner) show = false;
      }
      ...
      if (show && f.integrationStage && d.integrationStage !== f.integrationStage) show = false;

      tr.style.display = show ? "" : "none";
      if (show) visible++;
    }

    const empty = tbody.querySelector("tr.bbttcc-no-matches");
    if (empty) empty.style.display = (total > 0 && visible === 0) ? "" : "none";

    const countEl = root.querySelector("[data-role='filter-count']");
    if (countEl) countEl.textContent = total === visible ? `${total} hex${total === 1 ? "" : "es"}` : `${visible} / ${total} shown`;
  }
  ...
  async _onRender(ctx, opts) {
    ...
    root.addEventListener("click", (ev) => {
      const btn = act(ev, "open-travel-console");
      if (!btn) return;
      ev.preventDefault(); ev.stopPropagation();
      const tc = game.bbttcc?.ui?.travelConsole;
      if (tc) tc.render(true);
      else ui.notifications?.warn?.("Travel Console not ready yet.");
    }, { capture:true, signal:sig });
  ...
  // GM helper: always log the UUID when opening the Hex Editor (restores older debugging convenience)
    ...
    // Filter bar — input (live text) + change (selects) both update state and re-apply
    const onFilterInput = (ev) => {
      const el = ev.target;
      if (!(el instanceof HTMLElement)) return;
      const key = el.dataset?.filter;
      if (!key) return;
      this._bbttccFilters[key] = el.value ?? "";
      this._applyFilters(root);
    };
    root.addEventListener("input",  onFilterInput, { capture:true, signal:sig });
    root.addEventListener("change", onFilterInput, { capture:true, signal:sig });

    root.addEventListener("click", (ev) => {
      const btn = act(ev, "clear-filters");
      if (!btn) return;
      ev.preventDefault(); ev.stopPropagation();
      this._bbttccFilters = {
    ...
    // Restore filter UI from state (survives refresh / inline-edit re-renders)
    for (const [key, val] of Object.entries(this._bbttccFilters || {})) {
      if (!val) continue;
      const el = root.querySelector(`[data-filter="${key}"]`);
