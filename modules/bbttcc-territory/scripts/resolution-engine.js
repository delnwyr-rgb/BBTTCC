...
(() => {
  ...
  Hooks.once("ready", () => {
    ...
    terrApi.applyOutcome = async function applyOutcome({
      ...
      // Snapshot pre-state for the dossier ledger so we can emit a coherent
      // raid_outcome umbrella entry plus targeted modifier diffs.
      const _preStatus = tf.status || null;
      const _preMods   = Array.isArray(tf.modifiers) ? tf.modifiers.slice() : [];
      const _prePop    = tf.population ?? null;
      ...
      // Hex Dossier — emit one umbrella raid_outcome entry plus per-modifier
      // diffs so the timeline shows "Raid won: Liberation" with collapsed
      // child rows for each modifier added/removed and any status change.
      try {
        ...
        const outcomeLabel = outcome.label || String(outcomeKey || "Raid Outcome");
        const umbrellaKey = `raid:${hex.id}:${outcomeKey}:${Date.now()}`;
        if (typeof recorder === "function") {
          await recorder(hex, {
            kind: "raid_outcome",
            label: `Raid resolved: ${outcomeLabel}`,
            description: `Outcome '${outcomeKey}'${tier ? ` (tier: ${tier})` : ""} applied to hex.`,
            source: { activity: outcomeKey, factionId: String(factionId || ""), tier: tier || null },
            before: { status: _preStatus, modifiers: _preMods, population: _prePop },
            after:  { status: tf.status || null, modifiers: Array.isArray(tf.modifiers) ? tf.modifiers.slice() : [], population: tf.population ?? null },
            reversible: false,
            dedupeKey: umbrellaKey
          });
          // Also pre-write a dedupeKey for the integration enhancer's child
          // step so it doesn't double-log when applyIntegrationFromOutcome
          // fires next; we use a stable key based on the outcome+hex.
        }
        if (typeof modRecorder === "function" && outcome.hex) {
          const addedSet = new Set((outcome.hex.addModifiers || []).map(String));
          const removedSet = new Set((outcome.hex.removeModifiers || []).map(String));
          const beforeSet = new Set(_preMods.map(String));
          for (const m of addedSet) {
            if (!beforeSet.has(m)) {
              await modRecorder(hex, m, "added", { activity: outcomeKey, factionId: String(factionId || "") }, `${umbrellaKey}:add:${m}`);
            }
          }
          for (const m of removedSet) {
            if (beforeSet.has(m)) {
              await modRecorder(hex, m, "removed", { activity: outcomeKey, factionId: String(factionId || "") }, `${umbrellaKey}:rm:${m}`);
            }
          }
        }
      } catch (eL) { console.warn(TAG, "Dossier ledger write failed (raid_outcome):", eL); }
