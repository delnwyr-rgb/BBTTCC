  ...
  async function processRepairs(A, FFlags){
    ...
    for (const r of req.requests) {
      ...
      if (changed) {
        ...
        // Hex Dossier — record repair as a modifier removal.
        try {
          const modRecorder = game?.bbttcc?.api?.territory?.recordHexModifierTransition;
          if (typeof modRecorder === "function") {
            await modRecorder(doc, tag, "removed", { activity: "repair", factionId: A?.id || "" }, `repair:${doc.id}:${Date.now()}:${tag}`);
          }
        } catch (eL) { /* non-fatal */ }
      }
      done += 1;
    ...
    for (const scene of game.scenes ?? []) {
      ...
      for (const doc of drawings) {
        ...
        if (requests.cleanseCorruption) {
          ...
          // Hex Dossier — owner-action cleanse.
          try {
            ...
            if (typeof recorder === "function") {
              await recorder(doc, {
                ...
                description: "Darkness −2, Loyalty +1 applied via cleanse request.",
                source: { activity: "cleanse_corruption", factionId: owner?.id || "" },
                reversible: false
              });
            }
          } catch (eL) { /* non-fatal */ }
        ...
        if (requests.clearRockslide) {
          ...
          // Hex Dossier — record each removed bit as a modifier transition.
          try {
            const modRecorder = game?.bbttcc?.api?.territory?.recordHexModifierTransition;
            if (typeof modRecorder === "function") {
              const dedupePrefix = `clear_rockslide:${doc.id}:${Date.now()}`;
              for (const bit of removedBits) {
                await modRecorder(doc, bit, "removed", { activity: "clear_rockslide", factionId: owner?.id || "" }, `${dedupePrefix}:${bit}`);
              }
            }
          } catch (eL) { /* non-fatal */ }
        }
        ...
        if (requests.statusSet) {
          ...
          // Hex Dossier — record status change.
          try {
            const recorder = game?.bbttcc?.api?.territory?.recordHexImprovement;
            if (typeof recorder === "function" && oldStatus !== newStatus) {
              await recorder(doc, {
                kind: "owner_action",
                label: `Status: ${oldStatus} → ${newStatus}`,
                description: `Hex status changed via turn request.`,
                source: { activity: "status_set", factionId: owner?.id || "" },
          ...
          try {
            ...
            if (typeof recorder === "function") {
              await recorder(doc, {
                ...
                description: "Hex flagged as destroyed via turn request.",
                source: { activity: "destroy_hex", factionId: owner?.id || "" },
                before: { destroyed: false }, after: { destroyed: true },
                reversible: false
              });
            }
          } catch (eL) { /* non-fatal */ }
