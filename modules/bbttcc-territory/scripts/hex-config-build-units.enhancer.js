...
(() => {
  ...
  function injectButtons(app, html) {
    ...
    row.addEventListener("click", async (ev) => {
      ...
      try {
        ...
        if (action === "asset" && game?.bbttcc?.api?.territory?.townbuilder?.open) {
          await game.bbttcc.api.territory.townbuilder.open({ hexUuid: hexDoc.uuid });
          return;
        }
        ...
        if (res?.ok) {
          // Reflect the spend's modifier change IN PLACE. Re-rendering a V1
          // Dialog resets the form to its open-time HTML — it would discard
          // unsaved edits AND not even show the new modifier. Only touch the
          // checkbox this action writes, so other pending edits survive.
          try {
            const fresh = await fromUuid(hexDoc.uuid);
            const mods = fresh?.flags?.[MOD_T]?.modifiers;
            const modList = Array.isArray(mods) ? mods.map(String) : [];
            const syncModifier = (value) => {
              const box = root.querySelector(`input[name="modifiers"][value="${value}"]`);
              if (box) box.checked = modList.includes(value);
            };
            if (action === "fortify") syncModifier("Fortified");
            else if (action === "repair") syncModifier("Damaged Infrastructure");
          } catch (e) {
            warn("BU in-place refresh failed (harmless — flags are saved):", e);
          }
        }
      } catch (e) {
    ...
    // NOTE (2026-07-14): the GM Manual Edit panel is injected ONLY by
    // bbttcc-territory/scripts/main.js now (renderDialog hook + open-time
    // call). The duplicate injectGMPanel here raced it, and its Apply used
    // app.render(true), which reset the form and discarded unsaved edits.
}
