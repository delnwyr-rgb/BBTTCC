...
function renderRelationsCardHTML(actor){
  ...
  const canActDiplo = isGM || isOwner;

  // Diplomacy buttons — visible to faction owner / GM. Dialogs handle empty-state messaging.
  let diploButtons = "";
  if (canActDiplo) {
    const relApi = game?.bbttcc?.api?.factions?.relations;
    const hasAllied = (() => {
      if (!relApi?.list) return false;
      try {
        return (relApi.list(actor) || []).some(r => {
          if (r.tier !== "allied") return false;
          const back = relApi.get(game.actors.get(r.id), actor);
          return back === "allied";
        });
      } catch { return false; }
    })();
    const hasTradable = (() => {
      if (!relApi?.list || !relApi?.canTrade) return false;
      try {
        return (relApi.list(actor) || []).some(r => {
          const t = game.actors.get(r.id);
          return t && relApi.canTrade(actor, t).ok;
        });
      } catch { return false; }
    })();

    diploButtons = `
      <button type="button" class="button" data-action="diplo-send" ${hasAllied ? "" : "disabled"}
              data-tooltip="${hasAllied ? "One-way grant to a mutual-Allied counterparty." : "Requires at least one mutually-Allied counterparty."}">
        <i class="fas fa-handshake"></i> Send to Ally
      </button>
      <button type="button" class="button" data-action="diplo-trade" ${hasTradable ? "" : "disabled"}
              data-tooltip="${hasTradable ? "Bilateral exchange (tier-scaled friction)." : "Requires at least one mutual ≥Neutral counterparty."}">
        <i class="fas fa-balance-scale"></i> Open Trade
      </button>
    `;
  }

  const editButton = isGM
    ? `<button type="button" class="button" data-action="rels"><i class="fas fa-link"></i> Edit</button>`
    : "";
  const blurb = isGM
    ? "Relationship editing lives here for faction-to-faction standings and future hex/social links."
    : "Faction-to-faction standings (display-only — GM controls editing).";
  return `
    <div class="flexrow" style="justify-content:space-between; align-items:center; gap:.5rem; flex-wrap:wrap;">
      <div class="bbttcc-muted" style="flex:1; min-width:14rem;">${blurb}</div>
      <div class="flexrow" style="gap:.4rem; flex-wrap:wrap; justify-content:flex-end;">
        ${diploButtons}
        ${editButton}
      </div>
    </div>
...
class BBTTCC_RelationshipsConfig extends foundry.applications.api.ApplicationV2 {
  ...
  async _replaceHTML(a,b){
    ...
    save?.addEventListener("click", async ()=>{
      ...
      // Route through the Relations API so each tier delta fires the
      // bbttcc:relationship:changed hook (consumers: barter gating, future auto-nudge).
      const relApi = game?.bbttcc?.api?.factions?.relations;
      if (relApi?.set) {
        const before = getRelationsMap(this.actor);
        for (const [targetId, nextTier] of Object.entries(this._rels || {})) {
          const target = game.actors?.get(targetId);
          if (!target) continue;
          if (String(before[targetId] || "neutral") === String(nextTier)) continue;
          try {
            await relApi.set(this.actor, target, nextTier, { reason: "GM relationships dialog" });
          } catch (e) {
            console.warn("[bbttcc-relations] dialog write failed", e);
          }
        }
      } else {
        // Fallback: direct flag write (API not loaded — shouldn't happen post-ready).
...
function ensureRelationsMiniCardIn($root, actor){
  try {
    ...
    if (!hasBoundCard) {
      ...
      if ($contentTarget.length) {
        ...
        if (isGM) {
          ...
          $contentTarget.find('button').remove();
        }
      } else {
    ...
    $host.off("click.bbttccrels click.bbttccdiplo");
    if (isGM) {
      $host.on("click.bbttccrels", '[data-action="rels"]', ()=>{
        new BBTTCC_RelationshipsConfig(actor).render(true, { focus:true });
      });
    }
    // Diplomacy buttons (visible to GM + faction owner)
    if (isGM || actor?.isOwner) {
      $host.on("click.bbttccdiplo", '[data-action="diplo-send"]', ()=>{
        const App = game?.bbttcc?.apps?.AlliedSendApp;
        if (!App) return ui.notifications?.error?.("Allied Send app not loaded.");
        new App(actor).render({ force: true, focus: true });
      });
      $host.on("click.bbttccdiplo", '[data-action="diplo-trade"]', ()=>{
        const App = game?.bbttcc?.apps?.TradeApp;
        if (!App) return ui.notifications?.error?.("Trade app not loaded.");
        new App(actor).render({ force: true, focus: true });
      });
    }
