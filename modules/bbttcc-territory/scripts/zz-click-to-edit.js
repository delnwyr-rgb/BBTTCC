...
(() => {
  ...
  Hooks.once("init", () => {
    ...
    game.settings.register(MOD, "clickToEditModifier", {
      ...
      default: "shift" // Shift-click opens editor; plain clicks stay free for macros
    });
  ...
  // Don't steal clicks that belong to a token/tile/note — lets normal selection work.
  function placeableUnderWorldPoint(pt){
    const layers = [canvas.tokens, canvas.tiles, canvas.notes];
    for (const layer of layers) {
      ...
      for (let i = list.length - 1; i >= 0; i--){
        ...
        try {
          const local = p.toLocal(new PIXI.Point(pt.x, pt.y));
          if (p.hitArea?.contains?.(local.x, local.y)) return p;
          const w = p.hitArea?.width ?? p.w ?? p.width ?? 0;
          const h = p.hitArea?.height ?? p.h ?? p.height ?? 0;
          if (w > 0 && h > 0 && local.x >= 0 && local.y >= 0 && local.x <= w && local.y <= h) return p;
        } catch { /* skip */ }
      }
    }
    return null;
  ...
  async function handlePointerDown(evt, source){
    ...
    // Never steal a click that lands on a Token/Tile/Note — selection wins.
    if (placeableUnderWorldPoint(pt)) return;
  ...
  function attachListeners(){
    ...
    // 3) Document dblclick fallback only.
    // NOTE: do NOT attach renderer-view pointerdown fallback here.
    // In this scene setup it can resolve a second, offset/stale hit after stage already found the right hex.
    if (canvas?.app?.view) {
      try { canvas.app.view.removeEventListener("pointerdown", onViewPointerDown, { capture: true }); } catch (_e) {}
    }

    document.removeEventListener("dblclick", onDocumentDblClick, true);
    document.addEventListener("dblclick", onDocumentDblClick, true);

    log("click-to-edit ready — listeners attached (stage/document)");
  }
  ...
  const onViewPointerDown  = async (_evt)         => {};
