...
(() => {
  ...
  async function spendForAction({
    ...
    if (hexChanged) {
      ...
      // Hex Dossier — record build action with modifier diff against prior tf.
      try {
        ...
        const dedupePrefix = `build:${hexDoc.id}:${actionKey}:${Date.now()}`;
        if (typeof recorder === "function") {
          await recorder(hexDoc, {
            kind: "build",
            label: `Build action: ${actionKey} (−${cost} BU)`,
            description: `Faction ${A.name} spent ${cost} Build Units on ${actionKey}.`,
            source: { activity: `build:${actionKey}`, factionId: A.id || "" },
            before: { modifiers: Array.isArray(tf.modifiers) ? tf.modifiers.slice() : [] },
            after:  { modifiers: Array.isArray(newFlags.modifiers) ? newFlags.modifiers.slice() : [] },
            reversible: false,
            dedupeKey: dedupePrefix
          });
        }
        if (typeof modRecorder === "function") {
          const beforeSet = new Set(Array.isArray(tf.modifiers) ? tf.modifiers.map(String) : []);
          const afterSet  = new Set(Array.isArray(newFlags.modifiers) ? newFlags.modifiers.map(String) : []);
          for (const m of afterSet) {
            if (!beforeSet.has(m)) {
              await modRecorder(hexDoc, m, "added", { activity: `build:${actionKey}`, factionId: A.id || "" }, `${dedupePrefix}:add:${m}`);
            }
          }
          for (const m of beforeSet) {
            if (!afterSet.has(m)) {
              await modRecorder(hexDoc, m, "removed", { activity: `build:${actionKey}`, factionId: A.id || "" }, `${dedupePrefix}:rm:${m}`);
            }
          }
        }
      } catch (eL) { console.warn(TAG, "Dossier ledger write failed (build):", eL); }
    }
