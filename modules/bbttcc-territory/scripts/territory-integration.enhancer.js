...
(() => {
  ...
  async function applyStageEffects(hex, prevProgress, nextProgress) {
    ...
    try {
      ...
      // Hex Dossier — record any field that the stage-effect overlay actually
      // changed (size up/down, population shift, status normalization).
      try {
        const recorder = game?.bbttcc?.api?.territory?.recordHexImprovement;
        if (typeof recorder === "function") {
          const sizeKey  = `flags.${MODT}.size`;
          const popKey   = `flags.${MODT}.population`;
          const statusKey= `flags.${MODT}.status`;
          if (Object.prototype.hasOwnProperty.call(patch, sizeKey) && patch[sizeKey] !== size) {
            await recorder(hex, {
              kind: patch[sizeKey] === "outpost" ? "size_down" : "size_up",
              label: `Size: ${size || "(none)"} → ${patch[sizeKey]}`,
              description: `Wilderness hex auto-evolved on stage transition (${prevStage} → ${nextStage}).`,
              source: { activity: "stage_effects" },
              before: { size: size || null }, after: { size: patch[sizeKey] }, reversible: false
            });
          }
          if (Object.prototype.hasOwnProperty.call(patch, popKey) && patch[popKey] !== pop) {
            await recorder(hex, {
              kind: "owner_action",
              label: `Population: ${pop || "(none)"} → ${patch[popKey]}`,
              description: `Population auto-set on stage transition (${prevStage} → ${nextStage}).`,
              source: { activity: "stage_effects" },
              before: { population: pop || null }, after: { population: patch[popKey] }, reversible: false
            });
          }
          if (Object.prototype.hasOwnProperty.call(patch, statusKey) && patch[statusKey] !== rawStatus) {
            await recorder(hex, {
              kind: "owner_action",
              label: `Status: ${rawStatus} → ${patch[statusKey]}`,
              description: `Status normalized on stage transition.`,
              source: { activity: "stage_effects" },
              before: { status: rawStatus }, after: { status: patch[statusKey] }, reversible: false
            });
          }
        }
      } catch (eL) { console.warn(TAG, "Dossier ledger write failed (stage_effects):", eL); }
    } catch (e) {
  ...
  async function applyIntegrationFromOutcome(args = {}, result) {
    ...
    try {
      ...
      // Hex Dossier — record the integration step. dedupeKey lets a parent
      // raid_outcome umbrella entry (resolution-engine) suppress a duplicate
      // if it fired in the same flow.
      try {
        const recorder = game?.bbttcc?.api?.territory?.recordHexImprovement;
        if (typeof recorder === "function") {
          const stageKey = stageKeyFromProgress(next);
          const stageLabels = { wild:"Wild", outpost:"Outpost", developing:"Developing", settled:"Settled", integrated:"Integrated" };
          await recorder(hex, {
            kind: "integration_step",
            label: `Integration ${prev} → ${next} (${stageLabels[stageKey] || stageKey})`,
            description: `Outcome '${outcomeKey}' applied${tier ? ` (tier: ${tier})` : ""}.`,
            source: { activity: outcomeKey, tier: tier || null },
            before: { progress: prev },
            after: { progress: next, stageKey },
            reversible: false,
            dedupeKey: `integration:${hex.id}:${outcomeKey}:${prev}->${next}`
          });
        }
      } catch (eL) { console.warn(TAG, "Dossier ledger write failed (integration_step):", eL); }
    } catch (e) {
