...
// Rest") with RFI canon AND bakes in the per-use callout. Idempotent.
// Run on actor copies too so live characters get the cleaned text.
// ─────────────────────────────────────────────────────────────────────────────
(async () => {
  const SENTINEL = "data-ft-canon-echo-diver=\"v1\"";
...
` + calloutFor({
  ...
  for (const pack of game.packs) {
    ...
    try {
      ...
      for (const doc of docs) {
        ...
        } else if (CANON[doc.system?.identifier ?? ""]) {
          packSkip++;
        }
      }
  ...
  ui.notifications.info(`Echo-Diver scrub: pack ${packOK} updated (${packSkip} already canon), ${actorOK} actor items updated across ${actorTouched} actors.`);
  console.log("DONE");
})();
