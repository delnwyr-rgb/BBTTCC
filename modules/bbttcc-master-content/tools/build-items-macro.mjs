#!/usr/bin/env node
// build-items-macro.mjs
// Emits tools/animate-master-content-items.macro.js — a non-destructive, DRY_RUN-default
// in-world macro that stamps flags.autoanimations onto BBTTCC master-content weapons +
// wondrous use-items that lack it, and upgrades GENERIC Foundry placeholder icons
// (icons/*) to BBTTCC button icons. Never touches items that already have AA or use
// custom/button art. Keyed by item NAME so it runs in every world (both worlds are in
// sync for this module pack). Live is canonical (repo pack is stale) — this macro edits
// the live compendium in place, the same way the pack is normally maintained.
//
// Reads ./items-aa-map.json (validated: name -> {animTag, iconFile|null}).
...
const REF = JSON.parse(fs.readFileSync(path.join(HERE, "items-aa-map.json"), "utf8"));
const OUT = path.join(HERE, "animate-master-content-items.macro.js");
...
const macro = `// animate-master-content-items.macro.js — RUN IN-WORLD (GM). Run in BOTH worlds.
