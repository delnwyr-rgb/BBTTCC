// Menhirkin pack presence diagnostic. Read-only. Paste in F12 as GM.
(async () => {
  const EXPECTED = [
    "menhirkin",                        // species root
    "menhirkin_igneous_tier1",          // Magma Memory
    "menhirkin_metamorphic_tier1",      // The Thing You Were
    "menhirkin_sedimentary_tier1",      // Read the Strata
  ];
  ...
  console.log("=== Menhirkin Phase 3 batch — pack presence ===");
  ...
  ui.notifications[missing.length ? "warn" : "info"](
    missing.length
      ? `Menhirkin: ${present.size}/${EXPECTED.length} present, ${missing.length} MISSING.`
      : `Menhirkin: all ${EXPECTED.length} present.`
  );
  console.log("DONE");
})();
