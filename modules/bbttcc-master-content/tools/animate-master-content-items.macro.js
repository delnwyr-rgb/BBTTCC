...
// NON-DESTRUCTIVE: only ADDS Automated Animations to Bad Eden master-content weapons +
// wondrous use-items that lack it, and only SWAPS generic Foundry placeholder icons
// (icons/*) for Bad Eden button icons. Never alters items that already have AA or custom/
...
(async () => {
  ...
  const pack = game.packs.get("bbttcc-master-content.items")
            ...
            || game.packs.find(p => (p.title||"") === "Bad Eden Items");
  if (!pack) return ui.notifications.error("Could not find the Bad Eden master-content items compendium.");
