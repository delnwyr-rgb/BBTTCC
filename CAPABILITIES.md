...
- `._progression.*` (levelUp, tier math, path features — `ft-progression.js`), `._classAutomation.*` (~60 class dialog openers + `FEATURE_ROUTER`/`dispatchFeatureAction` — `ft-class-automation.js`), `.migrations.{run,list,stored,current}` (`migrations/index.js` — v1 baseline, v2 stamp-actor-kind, v3 repair-aptitude-grants).
...
### Hooks emitted
`fourththing.noiseChanged` (`:966`) · `fourththing:itemAnimated` (`:1583`) · `fourththing.manifestationCast` (`:7851`, payload incl. `reachPath`, `success`) · `fourththing.overshoot` (`:13773`) · death ladder: `enteredLastStand/stabilized/thresholdCrossed/redeemed/reincarnationPending` (`:16471-16711`) · `bloodDebtChanged/Cleared` (`:16747/:16763`) · `targetLeftReach` (`:17779`) · `revivedFromDying` (`:17900`) · `featureDispatched` (`ft-class-automation.js:7209`) · `bbttcc:rig:damaged/destroyed` (`:15502/:15505`) · `bbttcc:enlightenmentChanged` (`bbttcc-bridge.js:1854`).
...
- `openBuilder/getActiveCampaignId/setActiveCampaignId`; post-build: `openBeatDialog()`, `audio.*` (BeatAudioManager), `stopBeatNarration`, `show/closePlayerFacingBeatDialog`, `_lastEncounterCtx` (stash written by encounters).
...
- Choices: `label/description/prompt/style`, **`next`/`failNext`**, checks (`checkStat/checkDC/checkMode/checkPrompt`, GM adjudication), OP support (`support/supportSpend/supportOpKey`, desperation gate, HP-sacrifice), `politicalTags`.
...
`_launcher.playScenario` (`scene.launcher.js:1627`): per step activate scene → generic `scenario.spawn.actors[]` → **`spawnFactionPCs`** (2026-08-27: roster-driven, participating factions from ctx, skips PCs already on scene, bottom-center row, FRIENDLY) → bespoke `spawner.run` (registry has ONE entry: `bandit_ambush_standard`, hardcoded ids). Then outcome prompt → war log → auto-return (skipped if the beat path dived). `applyWorldEffectsForOutcome` (~1300 lines of per-scenario switches) divides/multiplies opBank by OP_TO_MARKS on read/save.
...
No settings (reads `bbttcc-campaign.encounterTables` cross-module), no hooks emitted, no tools, no styles. ⚠ declared circular dependency travel↔encounters.
...
**Tool families** (see survey 2026-08-27 for the full 202-entry catalog): AA animation stampers [DRY_RUN,GM] · **Courtly**: `courtly-secret-forge.macro.js` (interactive secret-template builder — rows drag onto mal-voice personas), `create-courtly-secrets` (3-sample seeder), `seed-courtly-maneuvers` [D] · maneuver catalog/balance sync ×14 · **`tableau-tune.macro.js`** + elevation variant (live tableau tuning panels) · leygates/harvest-node setup · gear/item creators ×20 · rigs/bosses/bestiary seeders ×17 · pricing stamps · class/path/doctrine authoring ×30 (incl. 9 prose-body rewrites) · effects/triggers/passives stampers · D&D-vocab scrubs (phase 2–7) · ancestry seed-or-scrub consoles · pack plumbing (`compile-loose-packs`, `probe-pack-duplicates`) · targeted playtest repairs · **Steward Gauntlet** 3-phase harness · read-only audits (`audit-aa-coverage`, `price-audit`, `audit-maneuvers-fire-mode`, ...).
...
Emits `bbttcc:epic:converged / :presence:changed / :malkuthAligned` (🟡 zero external listeners); listens `bbttcc:territory:hexUpdated`, `bbttcc:advanceTurn:end`, `bbttcc:spark:deposited/repaired`. UI: Ascension sheet panel, faction Presence chip, World-Health chip on Campaign Overview. Hidden settings `protagonistFactions/malkuthAligned`. Specs `~/EPIC_P*_SPEC.md`. 🟡 `alignHex/integrateSpark` have no UI callers; `ascent` band computed but unused (release 2).
...
Emits `bbttcc:spark:{corrupted, deposited, repaired}`. Listens **`bbttcc:beat:resolved`** (routes `beat.sparkLink` → spark API; payload schema at `tikkun-beat-listener.enhancer.js:11-19`). UI: character-sheet tab, GM faction-sheet "⚒ Spark Repair Ledger" / "→ Pending Deposits" buttons, Ritual Console AppV2, corruption flavor cards. No settings.
Tools (4): `load-sparks-pack` ⚠ **DRY_RUN=false (armed)** · tikkun gauntlet ×3 (static-audit read-only / forge / runner). ⚠ Structural smell: three files race to populate one namespace (api/hotfix/enhancer, all install-if-absent).
...
### 🔴 Broken wires — OPEN
4. **RULED (2026-08-28): future arc, design first.** (system) `system.darkness{value,taint,fragments}` has ZERO readers; `system.tikkun` has one fallback reader. Schema-only friction tracks kept as flagged seams until the personal-Darkness engine is designed (ties: enlightenment `darknessSpikes` perk, qliphothic play, Epic thaumiel path). Radiation is the fully-wired reference pattern.
