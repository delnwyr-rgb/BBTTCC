...
/* ------------------------------------------------
   Sephirot discovery (world + packs + canonical merge)
--------------------------------------------------- */
function keyFromName(n){ return String(n||"").toLowerCase().trim().replace(/[^\p{L}]+/gu,""); }

async function buildSephirotList() {
  const foundByName = new Map(); // Name (display form) -> {uuid,name}

  // 1) World items — exact 10 or anything tagged
  for (const it of game.items?.contents ?? []) {
    ...
    if (/^(Keter|Chokhmah|Binah|Chesed|Gevurah|Tiferet|Netzach|Hod|Yesod|Malkuth)$/i.test(name)) {
      ...
      if (cat && /sephiro/i.test(cat) && name) foundByName.set(name, { uuid: it.uuid, name });
    }
  }

  // 2) Compendiums — prefer canonical pack if present, else heuristic search
  ...
  for (const pid of packsToCheck) {
    try {
      ...
      for (const e of idx) {
        const nm = e?.name ?? "";
        if (!nm) continue;
        if (/^(Keter|Chokhmah|Binah|Chesed|Gevurah|Tiferet|Netzach|Hod|Yesod|Malkuth)$/i.test(nm) || /sephiro/i.test(nm)) {
          const uuid = `Compendium.${pack.collection}.${e._id}`;
          if (!foundByName.has(nm)) foundByName.set(nm, { uuid, name: nm });
        }
      }
    } catch {/* noop */}
  }

  // 3) Canonical baseline — ALWAYS include all ten
  const CANON = [
    "Keter","Chokhmah","Binah","Chesed","Gevurah","Tiferet","Netzach","Hod","Yesod","Malkuth"
  ];
  for (const name of CANON) {
    if (!foundByName.has(name)) {
      // Lightweight placeholder (uuid = key) so dropdown is complete even if no items
      foundByName.set(name, { uuid: keyFromName(name), name });
    }
  }
...
function getModifierEffects(modifiers = []) {
  ...
  if (set.has("supply line vulnerable")) { bump(-0.15); }
  return { mAll, mTrade };
}
    ...
    buttons: {
      save: {
        ...
        callback: async (jq) => {
          ...
          // Resolve selected Sephirot label for bonuses (+ canonical key)
          ...
          const selKey = keyFromName(selName); // <-- canonical key we’ll persist
          ...
          const upd = {
            ...
            // Alignment (persist ALL forms so every app resolves quickly)
            [`flags.${MOD}.sephirotUuid`]: selUuid,
            [`flags.${MOD}.sephirotName`]: selName,
            [`flags.${MOD}.sephirotKey`]:  selKey,   // <-- NEW: canonical key (keter…malkuth)

            // Keep notes/modifiers
