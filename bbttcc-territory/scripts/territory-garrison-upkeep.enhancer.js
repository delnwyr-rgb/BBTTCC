...
//     • Status (contested, occupied, claimed)
//     • Modifiers (Hostile/Loyal Population, Well-Maintained, Fortified, Trade Hub,
//                  Damaged Infrastructure, Radiation Zone, etc.)
// - Attempts to drain the appropriate OP buckets from faction opBank
// - On partial/unpaid upkeep, queues penalties into faction + hex turn.pending
// - Writes a brief War Log entry & optional GM card
//
// This file is intentionally self-contained and cautious: if anything is missing, it
// will log warnings but not throw hard errors.
//
// Option D: “Drain based on appropriate OP buckets per hex type”
// --------------------------------------------------------------

(() => {
  const TAG  = "[bbttcc-garrison]";
  ...
  // ------------------------------
  // Canon-inspired tables
  // ------------------------------

  // Base upkeep vectors per hex "type".
  // These are BEFORE phase/outcome/hostility/size multipliers.
  const BASE_BY_TYPE = {
    fortress:   { violence:1, nonlethal:1, logistics:1 },
    port:       { economy:1, logistics:1, diplomacy:1 },
    temple:     { faith:1, softpower:1, culture:1 },
    farm:       { economy:1, logistics:0.5 },
    mine:       { economy:1, nonlethal:0.5, logistics:0.5 },
    factory:    { economy:1, nonlethal:0.5, logistics:0.5 },
    research:   { intrigue:1, economy:0.5, softpower:0.5 },
    settlement: { economy:1, softpower:1, nonlethal:0.5, diplomacy:0.5 },
    city:       { economy:1, softpower:1, nonlethal:0.5, diplomacy:0.5 },
  ...
  // Hostility / Loyalty
  const HOSTILITY_MULT = 1.5;
  const LOYALTY_MULT   = 0.7;

  // ------------------------------
  // Core cost calculation per hex
  // ------------------------------
  ...
  function combineMult(base, mults = []) {
    let m = base;
    for (const f of mults) m *= f;
    return m;
  }
  ...
  function roundVector(vec) {
    ...
    for (const [k, v] of Object.entries(vec || {})) {
      const n = N(v);
      if (n <= 0) continue;
      out[k] = Math.ceil(n); // always round up so costs aren’t silently zeroed
    }
    return out;
  }
  ...
  function inferPhaseFromIntegration(integ) {
    ...
    if (prog >= 4) return "short_integration";
    return "occupation";
  }
  ...
  function computeHexUpkeep(tf, integ) {
    ...
    const finalMult = combineMult(1.0, [
      outMult,
      easeMult,
      extraMult,
      sizeMult,
    ...
    vec = scaleVector(vec, finalMult);

    // For full_integration with "good" outcomes, we want effectively 0 upkeep:
    if (phase === "full_integration" &&
  ...
  async function runGarrisonUpkeepForFaction(factionActor, hexes) {
    ...
    for (const d of hexes) {
      ...
      if (hasUnpaid) {
        ...
        const hexPendDelta = {
          ...
          // Optional: apply small morale hit locally, tracked via "localMorale" if you use it
          // localMoraleDelta: (hexPendBase.localMoraleDelta || 0) - 1,
        };
    ...
    console.log(TAG, `Upkeep processed for faction ${A.name}`, { bank, anyUnpaidGlobal });
  }
  ...
  async function runGarrisonUpkeep(args = {}) {
    try {
      ...
      if (!terrApi) {
        console.warn(TAG, "No territory api found; skipping garrison upkeep.");
        return;
      }

      // Group hexes by owning faction
      const byFaction = {};
      ...
      for (const sc of game.scenes ?? []) {
        for (const d of sc.drawings ?? []) {
          ...
          // Only care about hexes with integration data or contested/occupied status
  ...
  Hooks.once("ready", () => {
    ...
    if (terr.__bbttccGarrisonWrapped) {
      console.log(TAG, "Garrison wrapper already installed.");
      return;
    }
